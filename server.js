...
/** Render / panellerde sık görülen: tırnak, BOM, başta/sonda boşluk, https:// ile başlayan yanlış yapıştırma */
...
if (MONGODB_URI && !mongoUriLooksValid(MONGODB_URI)) {
  console.error('HATA: MONGODB_URI geçersiz. Değer mongodb:// veya mongodb+srv:// ile başlamalı.');
  console.error('Atlas’ta “Drivers” altından kopyalanan tam connection string olmalı; https://, tırnak veya gereksiz metin olmamalı.');
  const preview = MONGODB_URI.length > 80 ? MONGODB_URI.slice(0,80) + '…' : MONGODB_URI;
  console.error('Şu an okunan başlangıç:', JSON.stringify(preview));
}
...
const sessionSecret = process.env.SESSION_SECRET || 's3cr3t_default_key';
const sessionMaxAgeMs = 3 * 60 * 60 * 1000;
const mongoUriForSession = mongoUriLooksValid(MONGODB_URI) ? MONGODB_URI : '';
const sessionMiddleware = session({
  secret: sessionSecret,
  resave: false,
  saveUninitialized: false,
  cookie: { maxAge: sessionMaxAgeMs, secure: process.env.NODE_ENV === 'production' },
  ...(mongoUriForSession
    ? {
        store: MongoStore.create({
          mongoUrl: mongoUriForSession,
...
if (mongoUriForSession) {
  console.log('Oturumlar MongoDB üzerinde saklanıyor (connect-mongo).');
} else if (MONGODB_URI && !mongoUriLooksValid(MONGODB_URI)) {
  console.warn('MONGODB_URI şeması geçersiz; oturumlar geçici olarak bellekte (connect-mongo hata üretmesin diye).');
} else {
  console.warn('MONGODB_URI yok; oturumlar bellekte (yalnızca geliştirme için uygun).');
}
...
function parseCreditName(input) {
...
// --- MongoDB connection ---
if (!MONGODB_URI) {
  console.error('HATA: MONGODB_URI tanımlı değil. Render Environment veya .env içinde Atlas connection string ekleyin.');
} else if (!mongoUriLooksValid(MONGODB_URI)) {
  console.error('HATA: MONGODB_URI biçimi hatalı; mongoose.connect çağrılmadı (yukarıdaki açıklamaya bakın).');
} else {
...
app.get('/', async (req, res) => {
  try {
    ...
    const sonDakika = await News.find({ ...categoryFilter('Son Dakika'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
    const gundem = await News.find({ ...categoryFilter('Gündem'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
    const ekonomi = await News.find({ ...categoryFilter('Ekonomi'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
    const spor = await News.find({ ...categoryFilter('Spor'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
    const siyaset = await News.find({ ...categoryFilter('Siyaset'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
    const yasam = await News.find({ ...categoryFilter('Yaşam'), placement: { $in: ['homepage', 'both'] } }).sort({ createdAt: -1 }).limit(9);
...
app.post('/api/news', requireAuth, requireMongo, upload.single('image'), async (req, res) => {
  try {
    ...
    const data = {
      ...
      sourceName: parseCreditName(req.body.sourceName),
      reporterName: parseCreditName(req.body.reporterName),
